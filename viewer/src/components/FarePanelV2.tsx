...
import { HelpCircle, ChevronDown, Moon, BedDouble, Armchair, CheckCircle2, ChevronRight } from 'lucide-react';
...
/** Compute the minimum combined price across all segments of a multi-leg journey */
export function getMultiLegMinPrice(solution: SimplifiedSolution): number | null {
    if (solution.offers.length > 0) return solution.offers[0].price;
    // For multi-leg, sum the cheapest fare per segment
    let total = 0;
    for (const seg of solution.segments) {
        if (!seg.offers.length) return null;
        const cheapest = Math.min(...seg.offers.map(o => o.price));
        total += cheapest;
    }
...
export function FarePanel({ solution, onSelectionChange }: FarePanelProps) {
    ...
    // ── STANDARD MODE ─────────────────────────────────────────────────────────
    // For multi-leg: track which leg is "expanded" for selection
    const segCount = solution.segments.length;
    const completedSegs = solution.segments.filter(seg => segmentSelections[seg.id]?.flexibility != null).length;
    const runningTotal = (() => {
        let t = 0;
        solution.segments.forEach(seg => {
            const sel = segmentSelections[seg.id];
        ...
        <div>
            ...
            {hasMultipleSegments && (
                <div style={{
                    ...
                    marginBottom: 20,
                    padding: '10px 14px',
                    background: completedSegs === segCount ? C.successLight : '#F8F8FF',
                    border: `1px solid ${completedSegs === segCount ? '#A7F3D0' : '#D8D8F0'}`,
                    borderRadius: 9,
                }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {completedSegs === segCount
                            ? <CheckCircle2 style={{ width: 15, height: 15, color: C.success, flexShrink: 0 }} />
                            : <div style={{ width: 15, height: 15, borderRadius: '50%', border: '2px solid #A0AEC0', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#A0AEC0' }} />
                            </div>
                        }
                        <div>
                            <div style={{ fontSize: 12, fontWeight: 700, color: completedSegs === segCount ? C.success : '#4A4A8A' }}>
                                {completedSegs === segCount
                                    ? 'All fares selected!'
                                    : `Select a fare for each leg (${completedSegs}/${segCount} done)`
                                }
                            </div>
                            <div style={{ fontSize: 11, color: completedSegs === segCount ? '#0A9A5B99' : '#6B6BAA', marginTop: 1 }}>
                                {completedSegs === segCount
                                    ? `Total: €${runningTotal.toFixed(2)}`
                                    : 'Each leg may have a different carrier and fare options.'
                                }
                            </div>
                        </div>
                    </div>
                    {completedSegs > 0 && completedSegs < segCount && (
                        <div style={{ fontSize: 12, fontWeight: 700, color: '#4A4A8A' }}>
            ...
            {solution.segments.map((segment, segIdx) => {
                ...
                const isSegComplete = sel.flexibility != null;
                const selectedOffer = classOffers.find(o => o.id === sel.flexibility);
                const isLastSeg = segIdx === solution.segments.length - 1;

                return (
                    <div key={segment.id} style={{
                        marginBottom: isLastSeg ? 0 : 24,
                        borderRadius: 12,
                        border: hasMultipleSegments
                            ? `1.5px solid ${isSegComplete ? '#A7F3D0' : C.borderInput}`
                            : 'none',
                        overflow: 'hidden',
                    }}>
                        {/* ── Leg header (multi-seg only) ── */}
                        {hasMultipleSegments && (
                            <div style={{
                                display: 'flex',
                                alignItems: 'center',
                                ...
                                gap: 8,
                                padding: '12px 14px',
                                background: isSegComplete ? C.successLight : '#F5F5FF',
                                borderBottom: `1px solid ${isSegComplete ? '#A7F3D0' : '#E0E0F0'}`,
                            }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{
                                        ...
                                        background: isSegComplete ? C.success : C.primary,
                                        fontSize: 11, fontWeight: 800, color: '#fff',
                                        flexShrink: 0,
                                    }}>
                                        {isSegComplete ? '✓' : segIdx + 1}
                                    </div>
                                    <div style={{ minWidth: 0 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                                            <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>
                                                Leg {segIdx + 1} of {segCount}
                                            </span>
                                            <ChevronRight style={{ width: 12, height: 12, color: C.textSec }} />
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2, flexWrap: 'wrap' }}>
                                            ...
                                            {segment.trainNumber && (
                                                <span style={{ fontSize: 11, color: C.textSec }}>#{segment.trainNumber}</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                {/* Selected fare summary on the right */}
                                {isSegComplete && selectedOffer && (
                                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                        <div style={{ fontSize: 13, fontWeight: 700, color: C.success }}>
                                            €{selectedOffer.price.toFixed(2)}
                                        </div>
                                        <div style={{ fontSize: 11, color: '#0A9A5B99' }}>
                                            {selectedOffer.comfort} · {selectedOffer.tariffName}
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <div style={{ padding: hasMultipleSegments ? '14px 14px 16px' : '0' }}>
                            {/* CLASS TABS */}
                            <ClassCarouselStyle />
                        ...
                        {/* ── "Next leg" nudge between legs ── */}
                        {hasMultipleSegments && !isLastSeg && isSegComplete && (
                            <div style={{
                                margin: '0 14px 16px',
                                padding: '8px 12px',
                                background: '#F0F4FF',
                                ...
                                Now select your fare for Leg {segIdx + 2} below
                            </div>
                        )}
                    </div>
                );
            ...
            {/* ── Overall total for multi-leg ── */}
            {hasMultipleSegments && isComplete() && (
                <div style={{
                    marginTop: 16,
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    <div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: C.primary }}>Total for all {segCount} legs</div>
                        <div style={{ fontSize: 11, color: '#D0105A99', marginTop: 1 }}>
                            {solution.segments.map((seg, i) => {
                                const o = seg.offers.find(o => o.id === segmentSelections[seg.id]?.flexibility);
                                return o ? `Leg ${i + 1}: €${o.price.toFixed(2)}` : null;
                            }).filter(Boolean).join(' + ')}
                        </div>
                    </div>
                    <span style={{ fontWeight: 800, color: C.primary, fontSize: 20 }}>€{runningTotal.toFixed(2)}</span>
                </div>
            )}
        </div>
    );
